...
| Audio transcription | Groq Whisper turbo ~$0.04/audio-hour; yt-dlp bandwidth/CPU noise | **~$0.004 per 5 min** (scales with `max_transcription_seconds`) |
...
- **Free — ≈ $0.60 worst case** (summaries $0.18 + chat $0.23 + transcription $0.24 +
  storage <$0.01); typical usage ≈ $0.10/mo. A bounded, cheap acquisition funnel.
- **Pro — ≈ $5.10 absolute worst case** (summaries 25×30×$0.0024 = $1.80 + chat
  100×30×$0.0005 = $1.50 + transcription 10×30×$0.008 = $2.40... capped ≈ $5.7 if all
  three are simultaneously maxed; at a realistic 30% utilization ≈ **$1.70/mo cost vs
  $5 revenue** → ~65% gross margin. Simultaneous 100% utilization of all limits is
  break-even by design (the limits *are* the cost ceiling).
- **Premium — ≈ $16 absolute worst case** (summaries $7.20 + chat $3.00 + transcription
  $4.80 + storage $0.12); at 10–15% typical utilization ≈ **$2/mo cost vs $12 revenue**.
  A month-long 100% abuser loses ~$4 — acceptable exposure for now; if it materializes,
  tighten `daily_summaries` in the admin or add a monthly hard cutoff when Paddle
...
| Limit | Enforced | How |
|---|---|---|
| Daily summaries / chats / transcriptions | Backend | `plans.services.enforce_daily_limit` — rolling 24h count of `UsageEvent` rows; structured 429 with `{code, reason, message, cta}`; charged only on success |
...
| Transcription duration | Backend | clamped to `max_transcription_seconds` |
