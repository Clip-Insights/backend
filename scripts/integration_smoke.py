...
from integrations.registry import get_llm, get_vectorstore, get_email, get_analytics
...
def main():
    ...
    vs = get_vectorstore()
    vs.add_texts(["hello world"], [{"youtube_url": "http://x"}])
    docs = vs.similarity_search("hello", k=1, filter={"youtube_url": "http://x"})
    assert len(docs) == 1
